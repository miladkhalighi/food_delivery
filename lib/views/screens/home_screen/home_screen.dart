...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget gridItemsView(HomeScreenController _homeController) {
    return SizedBox(
      ...
      height: Dimension.size.height/2.7,
      child: Obx(
          () => ListView.builder(
            physics: const BouncingScrollPhysics(),
            shrinkWrap: true,
            ...
            itemBuilder: (context,index)=>Padding(
              padding: EdgeInsets.fromLTRB(index == 0 ? Dimension.bodyMargin : 16,16,index == _homeController.items.length-1 ? Dimension.bodyMargin : 16,16),
              child: FoodItem(
              width: Dimension.size.width/2.5, item: _homeController.items[index]
        ),
            )
        ),
      ),
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context,index){
          return Padding(
            padding: EdgeInsets.fromLTRB(index==0 ? Dimension.bodyMargin : 8,8,index==_homeController.categories.length-1 ? Dimension.bodyMargin : 8,8),
            child: Obx(() =>
              CategoryItem(name: _homeController.categories[index], pressed: (){
                _homeController.categoryItemSelected.value = index;
                _homeController.getItems();
              },
               enabled: _homeController.categoryItemSelected.value == index ? true : false,
              ),
            ),
          );
      }),
  ...
  Widget headerText() {
    return Padding(
            padding: const EdgeInsets.fromLTRB(Dimension.bodyMargin,16,Dimension.bodyMargin,0),
